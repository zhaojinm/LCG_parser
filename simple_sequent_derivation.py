...
class UnfoldNode():
    ...
    def __repr__(self) -> str:
        return self.prim + str(self.num) + " "+ str(self.sign)+ " "+str(self.depth)
...
def parse_sequent(cat_list, empty_premises,get_matching):
    ...
    for l in range(2, len(unfolding_formulae)+1, 2):
        ...
        for s in range(1, len(unfolding_formulae)-l+2):
            ...
            #brackeing //todo
            if abs(unfolding_formulae[s-1].depth - unfolding_formulae[e-1].depth)==1 and \
                 unfolding_formulae[s-1].prim == unfolding_formulae[e-1].prim and unfolding_formulae[s-1].sign + unfolding_formulae[e-1].sign==1:
...
def parse_xml(folder="trn", empty_premises = False, get_matching=False):
    ...
    for ws,mt,r,s in zip(words[:last_idx],matching[:last_idx],right_[:last_idx],sentence[:last_idx]):
        ...
        rhs = r['cat']
        if rhs.count("/")+rhs.count("\\")==0:
            cur_str = " ".join(cur_cat_with_num)+" -> "+rhs
            cur_str = "".join([i for i in cur_str if not i.isdigit() and i!="_"])
            print(cur_str)
            # print(s['id'])
            # # print(cur_match_str)
            # f = parse_sequent(cur_cat_with_num + [rhs], cur_match, empty_premises, get_matching)
            # final_graphs =  f[-1][-1]
            # answer = False
            # same_match = False
            # if final_graphs:
